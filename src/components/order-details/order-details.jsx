...
const OrderDetails = () => {
    ...
    const orderLoadingError = useSelector(state => state.order.orderLoadingError);
    ...
    if (orderLoadingError === true) {
        return (
            <div className={curStyle.order_root}>
                <p className="text text_type_main-default">Ошибка создания заказа. Повторите заказ позже</p>
            </div>
        )
    }
    ...
    if (order !== null) {
        return (
            <div className={curStyle.order_root}>
                <p className={`text text_type_digits-large ${curStyle.order_number}`}>{order.number}</p>
